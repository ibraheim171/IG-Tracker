...
export default async function HealthPage() {
  ...
  let analytics: ReturnType<typeof analyticsServiceClient> | null = null;
  if (profile.roles.includes("admin")) {
    try { analytics = analyticsServiceClient(); } catch { analytics = null; }
  }
  const [items, slots, posts, board] = await Promise.all([
    supabase.from("items").select("id", { count: "exact", head: true }),
    supabase.from("publishing_slots").select("id", { count: "exact", head: true }),
    analytics ? analytics.from("ig_posts").select("media_id", { count: "exact", head: true }) : Promise.resolve({ count: null }),
  ...
  return <main className="page stack"><h1>فحص الاتصال</h1>
    ...
    <section className="card stack"><h2>عدد الصفوف</h2><p>العناصر: <span className="num">{(items.count ?? 0).toLocaleString("en-US")}</span></p><p>مواعيد النشر: <span className="num">{(slots.count ?? 0).toLocaleString("en-US")}</span></p>{profile.roles.includes("admin") ? <p>منشورات إنستغرام: <span className="num">{posts.count === null ? "—" : posts.count.toLocaleString("en-US")}</span></p> : null}</section>
