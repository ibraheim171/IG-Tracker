...
export async function GET(request: NextRequest) {
  ...
  const [posts, items, links] = await Promise.all([
    ...
    service.from("ig_item_links").select("item_id,media_id,linked_at,linked_by,reason,source"),
  ]);
  ...
  return jsonWithCookies({
    ...
    items: (items.data ?? []).filter((row) => !linkedItems.has(row.id) && !row.ig_media_id),
    links: links.data ?? [],
  }, 200, session);
}
